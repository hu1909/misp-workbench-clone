...
from app.repositories.correlations import (build_cidr_query, build_query,
                                           delete_correlations,
...
class TestBuildQuery:
    ...
    def test_term_match(self):
        ...
        assert {"term": {"uuid.keyword": "uuid-1"}} in query["query"]["bool"][
            "must_not"
        ]
        assert {"term": {"event_uuid.keyword": "event-1"}} in query["query"]["bool"][
            "must_not"
        ]
    ...
    def test_prefix_match(self):
        query = build_query(
            ...
            "evil.example.com",
            "prefix",
            self._settings(prefix_length=5),
        )
    ...
    def test_fuzzy_match(self):
        query = build_query(
            "uuid-1", "event-1", "evil.com", "fuzzy", self._settings(fuzziness="AUTO")
        )
...
class TestBuildCidrQuery:
    ...
    def test_ip_src_cidr(self):
        ...
        assert query["query"]["bool"]["must"] == [
            {"term": {"expanded.ip": "192.168.1.0/24"}}
        ]
...
class TestGetCorrelations:
    ...
    def test_no_filters_uses_match_all(self):
        ...
        with patch(PATCH, return_value=mock_os):
            result = get_correlations(
                CorrelationQueryParams(), page=1, from_value=0, size=10
            )
    ...
    def test_target_attribute_uuid_filter(self):
        ...
        call_body = mock_os.search.call_args.kwargs["body"]
        assert {"term": {"target_attribute_uuid.keyword": "attr-002"}} in call_body[
            "query"
        ]["bool"]["must"]
    ...
    def test_target_event_uuid_filter(self):
        ...
        call_body = mock_os.search.call_args.kwargs["body"]
        assert {"term": {"target_event_uuid.keyword": "event-bbb"}} in call_body[
            "query"
        ]["bool"]["must"]
...
class TestGetTopCorrelatedEvents:
    ...
    def test_filters_by_source_event_uuid(self):
        mock_os = MagicMock()
        mock_os.search.return_value = {
            "aggregations": {"by_target_event": {"buckets": []}}
        }
    ...
    def test_returns_all_stats(self):
        ...
        mock_os.search.side_effect = [
            # get_top_correlating_events
            {
                "aggregations": {
                    "by_source_event": {
                        "buckets": [{"key": "event-aaa", "doc_count": 15}]
                    }
                }
            },
            # get_top_correlating_attributes
            {
                "aggregations": {
                    "by_target_attribute": {
                        "buckets": [{"key": "attr-001", "doc_count": 8}]
                    }
                }
            },
        ]
...
class TestDeleteEventCorrelations:
    def test_deletes_by_event_uuid(self):
        ...
        assert (
            result["message"]
            == "Correlations for event event-aaa deleted successfully."
        )
